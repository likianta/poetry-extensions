...
def walk_dependencies(
    ...
    while dependencies:
        ...
        # print(requirement, locked_package, ':vs')
        
        if not locked_package:
            print(
                ':v3s',
                'ignore no available locked package '
                '(which was mostly caused by invalid python constraint)',
                '{} @ python_version {}'.format(
                    requirement, requirement.python_constraint
                ),
            )
            continue
...
def get_locked_package(
    ...
    #     print(dependency, repr(dependency.constraint))
    #     d = dependency
    #     p = candidates[0]
    #     print(
    #         p.python_constraint.allows_all(d.python_constraint),
    #         d.constraint.allows(p.version),
    #         d.is_same_source_as(p),
    #         p.python_constraint,
    #         d.python_constraint,
    #         d.source_type,
    #     )
    ...
    #     print(
    #         ':v4',
    #         'no compatible candidate, nor overlapping candidate! will fallback '
    #         'to the first candidate to avoid poetry walking failure.',
    #         dependency, candidates[0]
    #     )
    #     compatible_candidates = [candidates[0]]
    ...
    if overlapping_candidates:
        ...
        if not filtered_compatible_candidates:
            print(
                ':v4s',
                'cannot find valid candidate in overlapping case. ignored',
                dependency
            )
            return None
